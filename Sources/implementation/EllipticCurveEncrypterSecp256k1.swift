...
final class EllipticCurveEncrypterSecp256k1 {
    ...
    init() {
        context = secp256k1_bindings.secp256k1_context_create(UInt32(SECP256K1_CONTEXT_SIGN | SECP256K1_CONTEXT_VERIFY))!
    }

    deinit {
        secp256k1_bindings.secp256k1_context_destroy(context)
    }
    ...
    func sign(hash: Data, privateKey: Data) -> secp256k1_ecdsa_recoverable_signature? {
        ...
        let status = privateKey.withUnsafeBytes { key -> Int32 in
            hash.withUnsafeBytes { hash -> Int32 in
                secp256k1_bindings.secp256k1_ecdsa_sign_recoverable(context, &signature, hash.baseAddress!.assumingMemoryBound(to: UInt8.self), key.baseAddress!.assumingMemoryBound(to: UInt8.self), nil, nil) }
        }
    ...
    func export(signature: inout secp256k1_bindings.secp256k1_ecdsa_recoverable_signature) -> Data {
        var output = Data(count: 65)
        var recid = 0 as Int32
        _ = output.withUnsafeMutableBytes { output in
            secp256k1_bindings.secp256k1_ecdsa_recoverable_signature_serialize_compact(context, output.baseAddress!.assumingMemoryBound(to: UInt8.self), &recid, &signature)
        }
    ...
    func `import`(signature: Data) -> secp256k1_ecdsa_recoverable_signature {
        ...
        var sig = secp256k1_bindings.secp256k1_ecdsa_recoverable_signature()
    ...
    func publicKey(signature: inout secp256k1_bindings.secp256k1_ecdsa_recoverable_signature, hash: Data) -> secp256k1_pubkey? {
        precondition(hash.count == 32, "Hash must be 32 bytes size")
        ...
        var outPubKey = secp256k1_bindings.secp256k1_pubkey()
        let status = secp256k1_bindings.secp256k1_ecdsa_recover(context, &outPubKey, &signature, bytes)
    ...
    func export(publicKey: inout secp256k1_pubkey, compressed: Bool) -> Data {
        ...
        output.withUnsafeMutableBytes { pointer -> Void in
            secp256k1_bindings.secp256k1_ec_pubkey_serialize(context, pointer.baseAddress!.assumingMemoryBound(to: UInt8.self), &outputLen, &publicKey, compressedFlags)
        }
        return output
    }
}
